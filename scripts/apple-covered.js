...
export default class AppleCovered {
    ...
    draw = (redraw) => {
        ...
        //destructuring to access the settings in options more easily
        const  { options } = this
        ...
        options.paperStartPoint = geometric.pointTranslate(options.left, options.surfaceAngle, 35);
        options.paperEndPoint = geometric.pointTranslate(options.left, options.surfaceAngle, -35);
        ...
        if (!redraw) {
            ...
            options.paperTop = this.svg.append("line")
                .attr("class", "paper")

            options.paperBottom = this.svg.append("line")
                .attr("class", "paper")    
        ...
        options.paperTop
            .attr("x1", options.paperStartPoint[0] - 75)
            .attr("y1", options.paperStartPoint[1])
            .attr("x2", options.paperEndPoint[0] + 1) //make sure there's never a tiny gap that line can intersect
        ...
        options.paperBottom
            .attr("x1", options.paperEndPoint[0] - 75)
            .attr("y1", options.paperEndPoint[1])
            .attr("x2", options.paperEndPoint[0])
            .attr("y2", options.paperEndPoint[1]);     
    ...
    update = (options) => {
        ...
        //test if reflection line is going into top paper 
        options.paperTopIntersection = geometric.lineIntersectsLine([[options.reflectedPoint[0], options.reflectedPoint[1]], [options.end[0], options.end[1]]], [[options.paperStartPoint[0] - 75, options.paperStartPoint[1]], [options.paperEndPoint[0], options.paperStartPoint[1]]]);

        //test if reflection line is going into right paper 
        ...
        this.handleIntersections(options.paperTopIntersection, options.paperIntersection)
    }

    handleIntersections = (paperTop, paper) => {

        //reset the y of the reflection line if it intersects with the top paper
        if (paperTop) {
            this.options.reflectedLine
                .attr("y1", this.options.paperStartPoint[1])
        }
